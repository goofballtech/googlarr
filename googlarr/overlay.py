...
def calculate_target_size(
    ...
    if config['scale_by_face_size']:
        ...
        if config['use_same_size_for_both_eyes'] and standard_eye_size is not None:
        ...
        target_width = int(size_to_use[0] * config['eye_overlay_scale'])
        target_height = int(size_to_use[1] * config['eye_overlay_scale'])
...
def apply_overlay(
    ...
    try:
        ...
        target_width, target_height = calculate_target_size(config, eye_location, eye_index, standard_eye_size)
...
def process_image(
    ...
    if config['prevent_floating_eyes']:
        filtered_locations = [
            location for location in eye_locations 
            if location.confidence >= config['min_face_confidence']
        ]
        if len(filtered_locations) < len(eye_locations):
            logger.info(
                f"Filtered out {len(eye_locations) - len(filtered_locations)} "
                f"low confidence face detection(s) below threshold {config['min_face_confidence']}"
            )
    ...
    standard_eye_size = calculate_eye_size(eye_locations) if config['use_same_size_for_both_eyes'] else None
    
    for eye_loc in eye_locations:
        try:

            if False:
                cx, cy = eye_loc.face_center
                w, h = eye_loc.face_size

                # Calculate top-left corner of face box
                x1 = int(cx - w / 2)
                y1 = int(cy - h / 2)
                x2 = int(cx + w / 2)
                y2 = int(cy + h / 2)
